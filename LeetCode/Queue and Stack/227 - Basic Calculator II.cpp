    ...
    int calculate(string s) {
        ...
        while (!fixed_terms.empty()) {
            ...
            if (current.first == 0) {//숫자
                ...
                if (convert_stk.top().second == '(') {//빈 괄호 처리
                    postfix.push(pair<int, int>(0, 0));//0 삽입
                }
                ...
                if (convert_stk.empty()) convert_stk.push(current);//비어있으면 무조건 push
                    ...
                    if (convert_stk.top().second == '(') {
                        ...
                        while (!convert_stk.empty() && convert_stk.top().first == 1 && convert_stk.top().second != '(') {
                            switch (convert_stk.top().second) {
                            case '*':
                            case '/':
                                stack_prio = 2;
                                break;
